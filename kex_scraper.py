"""
KEX Express scraper ใช้ Pyppeteer (Puppeteer สำหรับ Python)
ดึงสถานะพัสดุจาก th.kex-express.com โดยตรง ไม่มีค่า API
"""
import asyncio
import re
...
async def scrape_kex(barcode: str) -> dict:
    ...
    try:
        ...
        try:
            await page.waitForSelector('li.status-line', {'timeout': 20000})
        except Exception:
            return {
                "barcode":   barcode,
                "status":    "unknown",
        ...
        () => {
            ...
            return [...items].map(item => {
                ...
                return { description: status, location, date, time,
        ...
        status, status_th = KEX_STATUS_MAP.get(
            latest_desc,
            ("in_transit", latest_desc)
        )

        # เรียงกลับให้เก่าสุดอยู่ก่อน
...
async def fetch_kex_tracking(barcode: str) -> dict:
    """Entry point — wrapper สำหรับเรียกจาก main.py"""
    return await scrape_kex(barcode)
...
if __name__ == "__main__":
    ...
    async def test():
        result = await fetch_kex_tracking(code)
        import json
